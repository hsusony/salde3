...
class _StockOutItem {
  ...
  _StockOutItem({required this.product, this.quantity = 1, this.notes});
}
...
class _StockOutScreenState extends State<StockOutScreen> {
  ...
  final List<_StockOutItem> _items = [];
              ...
              onPressed: () {
                ...
                if (quantity > 0 && quantity <= product.quantity) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                          'الكمية غير صحيحة. المخزون المتاح: ${product.quantity}'),
  ...
  Future<void> _saveTransaction() async {
    ...
    for (var item in _items) {
      final transaction = InventoryTransaction(
        ...
        notes: item.notes ?? _notesController.text,
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              const SizedBox(width: 12),
              Text('تم حفظ ${_items.length} عملية إخراج بنجاح'),
            ],
          ),
          backgroundColor: const Color(0xFF10B981),
